...
select sku,
	...
	url,
	release_year,
	release_month,insert into c2dwh_silver.earphones with temp as(
		select cast(sku as int) sku,
			name,
			case
				when price = '0' then null else cast(price as int)
			...
			url,
			case
				when release_date = '' then null else cast(release_date as int)
			end release_year,
			cast(updated_at as timestamp) updated_at,
			case
				when sound_tech = ''
...
select sku,
	...
	water_resistant,
	ports,
	runtime_hrs,insert into c2dwh_silver.earphones with temp as(
		select cast(sku as int) sku,
			name,
			case
				when price = '0' then null else cast(price as int)
